...
interface LayersModalProps {
  isOpen: boolean;
  onClose: () => void;
  assetsByPosition: Record<"front" | "left" | "right", Asset[]>;
  activePosition: "front" | "left" | "right";
  selectedAssetIndex: number | null;
  onSelectLayer: (pos: "front" | "left" | "right", idx: number) => void;
  onDeleteLayer: (pos: "front" | "left" | "right", idx: number) => void;
}
...
export function LayersModal({
  ...
  return (
    <>
      ...
      />
      <motion.div
        className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full bg-white rounded-t-2xl shadow-lg z-50 p-6 max-h-[90vh] overflow-y-auto"
        initial={{ y: "100%" }}
        animate={{ y: 0 }}
        exit={{ y: "100%" }}
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="text-sm truncate max-w-[120px]">
                        {asset.type === "text"
                          ? (asset.value as string).substring(0, 15) + ((asset.value as string).length > 15 ? "..." : "")
                          : asset.type === "upload"
                          ? (asset.value as File).name.substring(0, 15) + ((asset.value as File).name.length > 15 ? "..." : "")
                          : "Asset"}
                      </span>
                    </div>
                    <button
